            ...
            {examples.length > 0 ? (
              <div className="grid gap-3">
                {examples.map((example) => (
                  <Link
                    key={example.slug}
                    className="group grid gap-4 rounded-[1.6rem] border border-white/64 bg-white/70 p-4 text-left shadow-[0_14px_34px_rgba(15,23,42,0.055)] outline-none backdrop-blur-2xl transition-transform focus-visible:ring-2 focus-visible:ring-[var(--ring)]/45 active:scale-[0.995] sm:grid-cols-[minmax(0,1fr)_12rem]"
                    href={example.href}
                  >
                    <span className="min-w-0">
                      <span className="text-xs font-semibold uppercase text-foreground/42">
                    ...
                    <span className="flex flex-col justify-between gap-4 rounded-[1.1rem] border border-black/6 bg-white/68 p-3">
                      ...
                      <span className="text-sm font-semibold text-foreground">
                        {example.flowSteps.length} steps · {example.screens.length} screens
                      </span>
