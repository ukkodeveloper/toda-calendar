      ...
      {open ? (
        <div className="fixed inset-0 z-50 flex items-end justify-center">
          ...
          <motion.section
            ...
            className={cn(
              "relative z-10 flex max-h-[82dvh] w-full max-w-[34rem] flex-col overflow-hidden rounded-t-[28px] border border-black/6 bg-[var(--surface-panel)] text-foreground shadow-[0_-18px_52px_rgba(15,23,42,0.16)] backdrop-blur-2xl",
              className
            )}
            initial={reducedMotion ? { opacity: 0 } : { opacity: 0, y: 24 }}
            animate={reducedMotion ? { opacity: 1 } : { opacity: 1, y: 0 }}
            exit={reducedMotion ? { opacity: 0 } : { opacity: 0, y: 24 }}
            transition={reducedMotion ? { duration: motionTokens.duration.instant } : motionTokens.spring.sheet}
          >
              ...
              <header className="px-4 pt-2 pb-3 sm:px-5">
                <div className="grid grid-cols-[auto_1fr_auto] items-start gap-3">
                  ...
                  <div className="min-w-0 pt-1 text-center">
                    {title ? (
                      <h2 className="truncate text-[1.05rem] font-semibold tracking-[-0.025em]">
                        {title}
                      </h2>
                    ) : null}
                    {description ? (
